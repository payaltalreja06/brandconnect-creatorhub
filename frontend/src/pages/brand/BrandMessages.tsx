  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] flex">
      <div className={`w-full md:w-80 border-r border-border flex flex-col ${selectedThread ? "hidden md:flex" : "flex"}`}>
        <div className="p-4 border-b border-border"><h2 className="font-bold text-lg">Messages</h2></div>
        <div className="flex-1 overflow-auto">
          {threads.map((t) => (
            <div key={t.id} onClick={() => setSelectedThread(t)}
              ...
              {t.avatar.startsWith("http") ? <img src={t.avatar} className="w-10 h-10 rounded-full bg-muted" alt="" /> : <span className="text-2xl w-10 h-10 flex items-center justify-center">{t.avatar}</span>}
              ...
              {t.unread > 0 && <Badge variant="destructive" className="text-xs h-5 w-5 flex items-center justify-center p-0 rounded-full">{t.unread}</Badge>}
            </div>
          ))}
        </div>
      </div>
        ...
        {selectedThread ? (
          <>
            <div className="p-4 border-b border-border flex items-center gap-3">
              <Button variant="ghost" size="icon" className="md:hidden" onClick={() => setSelectedThread(null)}><ArrowLeft className="w-4 h-4" /></Button>
